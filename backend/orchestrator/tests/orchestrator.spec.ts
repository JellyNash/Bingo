...
import {
  deriveSeedHex, prngFromSeedHex, newShuffledDeck, initDrawState, drawNext,
  generateCard, standardPatterns, marksFromDraws, validateClaim, applyPenalty
} from "../src";
  ...
  it("columns respect ranges and free center", () => {
    ...
    for (let r = 0; r < 5; r++) {
      for (let c = 0; c < 5; c++) {
        ...
        if (c === 0) expect(v).toBeGreaterThanOrEqual(1), expect(v).toBeLessThanOrEqual(15);
        if (c === 1) expect(v).toBeGreaterThanOrEqual(16), expect(v).toBeLessThanOrEqual(30);
        if (c === 2) expect(v).toBeGreaterThanOrEqual(31), expect(v).toBeLessThanOrEqual(45);
        if (c === 3) expect(v).toBeGreaterThanOrEqual(46), expect(v).toBeLessThanOrEqual(60);
        if (c === 4) expect(v).toBeGreaterThanOrEqual(61), expect(v).toBeLessThanOrEqual(75);
      }
    }
  });
});
...
describe("Claim validation", () => {
  it("detects a completed row", () => {
    ...
    const rowNums = [0,1,2,3,4].map(i => card.grid[i]).filter(Boolean) as number[];
    const drawn = new Set<number>(rowNums);
