export function attachMediaController(socket, engine) {
    // Handle draw:next events - auto-play the drawn number
    const handleDrawNext = (data) => {
...
export function createMediaTester(engine) {
    return {
        testNumber: (n) => engine.playNumber(n),
...
if (import.meta.env.DEV) {
    window.mediaTest = (engine) => createMediaTester(engine);
}
